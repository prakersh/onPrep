"""Tests for C advanced-level (APUE) seed JSON files (16-21).

Validates structure, completeness, and correctness of APUE systems
programming seed data before it gets loaded into the database.
"""
...
REQUIRED_FILES = {
    '16_unix_file_io.json': {
        'concept_slug': 'unix-file-io',
        'expected_count': 15,
        'difficulty': 'advanced',
    },
    '17_files_directories.json': {
        'concept_slug': 'files-directories',
        'expected_count': 12,
        'difficulty': 'advanced',
    },
    '18_process_control.json': {
        'concept_slug': 'process-control',
        'expected_count': 18,
        'difficulty': 'advanced',
    },
    '19_signals.json': {
    ...
    '21_threads_synchronization.json': {
        'concept_slug': 'threads-synchronization',
        'expected_count': 15,
        'difficulty': 'advanced',
    },
}
...
class TestAdvancedSeedFilesExist:
    """Verify all 6 advanced seed files exist."""
...
class TestAdvancedSeedStructure:
...
class TestAdvancedQuestionFields:
...
class TestAdvancedAnswerFields:
...
class TestAdvancedMetaAlignment:
...
class TestAdvancedContentQuality:
    """Additional quality checks specific to APUE content."""

    @pytest.mark.parametrize('filename', REQUIRED_FILES.keys())
    def test_actual_code_contains_includes(self, filename: str) -> None:
        """APUE code should include system headers."""
        data = load_seed(filename)
        has_include = False
