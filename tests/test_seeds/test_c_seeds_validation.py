"""Tests to validate C seed JSON files conform to the expected schema and quality standards."""
...
SEEDS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'seeds', 'c')

EXPECTED_FILES = [
    ('01_data_types_variables.json', 'data-types-variables-storage', 15),
    ('02_operators_expressions.json', 'operators-expressions', 12),
    ('03_control_flow.json', 'control-flow', 12),
    ('04_functions_recursion.json', 'functions-recursion', 15),
    ('05_arrays_strings.json', 'arrays-strings', 15),
    ('06_pointers_fundamentals.json', 'pointers-fundamentals', 15),
    ('07_preprocessor_compilation.json', 'preprocessor-compilation', 11),
]
...
class TestCSeedFilesExist:
    """Verify all 7 seed files exist and are valid JSON."""
...
class TestCSeedAnswers:
    ...
    @pytest.mark.parametrize('filename,slug,count', EXPECTED_FILES)
    def test_code_language_is_c(self, filename: str, slug: str, count: int) -> None:
        data = load_seed_file(filename)
        for i, q in enumerate(data['questions']):
            for a in q['answers']:
                assert a['code_language'] == 'c', (
                    f'{filename} Q{i+1} {a["mode"]}: code_language must be "c", got "{a["code_language"]}"'
                )
    ...
    @pytest.mark.parametrize('filename,slug,count', EXPECTED_FILES)
    def test_tags_not_empty(self, filename: str, slug: str, count: int) -> None:
        data = load_seed_file(filename)
        for i, q in enumerate(data['questions']):
            assert len(q['tags']) > 0, f'{filename} Q{i+1}: tags must not be empty'
